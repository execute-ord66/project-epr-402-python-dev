...
# =============================================================================
# Memory-Efficient DynamicMixingDataset with Persistent Cache
# =============================================================================
class DynamicMixingDataset(IterableDataset):
    ...
    def _initialize_worker(self):
        """Initializes resources for each worker, like the cache."""
        # The cache is created here, so each worker gets its own.
        # This is crucial for multiprocessing.
        self.cache = FileCache(
            max_items=self.pool_size, dp_config=self.dp, log_callback=self.log
        )

    def _get_random_segment_from_cache(self):
        """
        Efficiently loads a segment using the persistent cache.
        Slow on cache miss, very fast on cache hit.
        """
        attempts = 0
        while attempts < 100:  # Limit attempts to find a valid segment
            stem_info = random.choice(self.stem_info_list)
            audio_path = stem_info["audio_path"]

            # Get processed data from the cache (fast if already loaded)
            y, rms_env = self.cache.get_processed_audio(audio_path)

            # Handle cases where file loading failed
            if y is None:
                attempts += 1
                continue

            if len(y) < self.segment_len_samples:
                attempts += 1
                continue

            # Find loud parts (very fast operation)
            rms_db = 20 * np.log10(np.maximum(rms_env, 1e-7))
            good_indices = np.where(rms_db > self.loudness_thresh)[0]

            if good_indices.size == 0:
                attempts += 1
                continue

            # --- The rest is the same fast, in-memory slicing ---
            chosen_frame = random.choice(good_indices)
            rms_hop = self.dp.get("rms_hop", 256)
            start_samples = chosen_frame * rms_hop
            if start_samples + self.segment_len_samples > len(y):
                start_samples = len(y) - self.segment_len_samples

            segment = y[start_samples : start_samples + self.segment_len_samples]

            f0_df = pd.read_csv(
                stem_info["f0_path"]
            )  # F0 CSVs are small, reading them is fast
            f0_df.columns = [c.strip() for c in f0_df.columns]
            start_time = start_samples / self.dp["sr"]
            end_time = (start_samples + self.segment_len_samples) / self.dp["sr"]

            padding = 0.1
            f0_segment_df = f0_df[
                (f0_df["time"] >= start_time - padding)
                & (f0_df["time"] <= end_time + padding)
            ]

            if f0_segment_df.empty:
                f0_times, f0_freqs = np.array([]), np.array([])
            else:
                f0_times = f0_segment_df["time"].values - start_time
                f0_freqs = f0_segment_df["frequency"].values

            return segment, (f0_times, f0_freqs)

        # Fallback if no valid segment can be found
        self.log(
    ...
    def __iter__(self):
        # Initialize the cache once per worker process.
        if self.cache is None:
            self._initialize_worker()

        # "Populating the pool" will now have a warm-up period.
        # It will be slow at first as the cache fills, then get progressively faster.
        pool = [
            self._get_random_segment_from_cache()
            for _ in tqdm(
                range(self.pool_size), desc="Warming up data cache", leave=False
            )
        ]

        for _ in range(self.steps_per_epoch):
            stems_to_mix = random.sample(pool, self.stems_per_mix)

            # Refill the pool with new segments
            for i in range(self.stems_per_mix):
                pool[random.randint(0, self.pool_size - 1)] = (
                    self._get_random_segment_from_cache()
                )

            # --- Mix audio (all fast operations) ---
            mixed_audio_np = np.zeros(self.segment_len_samples, dtype=np.float32)
            f0_data_list = []
            for audio_segment, f0_tuple in stems_to_mix:
                gain_linear = 10.0 ** (random.uniform(-3.0, 3.0) / 20.0)
                mixed_audio_np += audio_segment * gain_linear
                f0_data_list.append(f0_tuple)

            yield mixed_audio_np, f0_data_list
...
# =============================================================================
# Smart FileCache for Processed Data
# =============================================================================
class FileCache:
    """
    A smart, persistent cache that stores the RESULTS of expensive processing
    (resampling, RMS calculation) to avoid re-computation.
    """
    ...
    def __init__(self, max_items=256, dp_config=None, log_callback=print):
        ...
        self.dp = dp_config
        self.log = log_callback
        # This cache will be populated with tuples: (processed_audio_y, processed_rms_env)
        self.processed_audio_cache = OrderedDict()

    def get_processed_audio(self, audio_path):
        ...
        try:
            ...
            rms_env = precompute_rms_env(y, frame_length=rms_frame, hop_length=rms_hop)

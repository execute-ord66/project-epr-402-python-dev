...
def get_default_config():
    ...
    return {
        ...
        "data_params": {
            ...
            "bins_per_octave": 60, "n_octaves": 6, "gaussian_sigma": 4.0,
...
class ConvViTForSalience(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.conv_frontend = nn.Sequential(
            # Layer 1
            nn.Conv2d(input_channels, conv_channels[0], kernel_size=5, stride=2, padding=2),
            nn.BatchNorm2d(conv_channels[0]),
            nn.GELU(),
            # Layer 2
            nn.Conv2d(conv_channels[0], conv_channels[1], kernel_size=5, stride=2, padding=2),
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.Linear(d_model, n_bins * patch_width)
        )
    ...
    def forward(self, x):
        ...
        x = self.conv_frontend(x) # -> (b, patch_dim, num_patches_freq, num_patches_time)
        ...
        salience_map = rearrange(salience_flat, 'b (f t) -> b 1 f t', f=self.n_bins, t=self.patch_width)
    ...
    def process_and_cache_group(self, track_stems):
        ...
        for stem in valid_stems_for_f0:
            ...
            if is_choralsynth_style:
                ...
                active_mask = (interp_freqs >= 1.5*self.dp['fmin']) & (interp_confidence > 0.5)
            else:
                dataset_folder = self.get_dataset_folder(stem)
                if not dataset_folder:
                    continue
        ...
        f0_map_hi = gaussian_filter(f0_map_hi, sigma=(self.dp['gaussian_sigma']*np.sqrt(SS_F), 0), mode='constant')
...
class PatchDataset(Dataset):
    ...
    def __init__(self, track_groups, root_dir, cache_dir, config, is_train, log_callback):
        ...
        # --- MODIFICATION START: Initialize Augmenter and Mixup ---
        self.augmenter = None
        self.mixup_alpha = 0.0  # Default to off
        if self.is_train:
            self.augmenter = SalienceAugment() # Using default SpecAugment values
            
            # Get mixup alpha from config. If > 0, mixup is enabled.
            # A common value is 0.4. Set to 0.0 in config to disable.
    ...
    def __getitem__(self, idx):
        # Step 1: Retrieve the primary data sample
        ...
        # Slice the patch from the full data
        ...
        # Step 2: Apply augmentations only during the training phase
        if self.is_train:
            # MIXUP: Apply with a 50% probability if alpha > 0
            if self.mixup_alpha > 0 and random.random() < 0.5:
                ...
                mix_f0_patch = torch.from_numpy(mix_f0_patch.copy()).unsqueeze(0).float()
                
                # Sample the mixing coefficient from a beta distribution
            ...
            # SPECAUGMENT: Apply after mixup using the augmenter class
            if self.augmenter:
...
class DatasetManager:
    ...
    def generate_data_split(self, config):
        """
        Generates a new train/validation split and returns the UPDATED config.
        This should be called ONCE before a new training run starts.
        """
        self.log("Generating a new random train/validation split.")
        ...
        # 3. UPDATE AND RETURN THE CONFIG OBJECT
        config['data_params']['train_groups'] = train_groups
        config['data_params']['val_groups'] = val_groups
        self.log(f"Split generated: {len(train_groups)} training groups, {len(val_groups)} validation groups.")
        return config

    def get_dataloaders(self, config):
        """
        Creates and returns training and validation DataLoaders BASED ON A GIVEN SPLIT.
        """
        class EmptyDataset(Dataset):
            def __len__(self): return 0
            def __getitem__(self, idx): raise IndexError

        # Get the pre-defined split from the config
        ...
        if not train_groups or not val_groups:
            self.log("ERROR: Config is missing 'train_groups' or 'val_groups'. Cannot create dataloaders.")
        ...
        if len(train_dataset) == 0:
            self.log(f"ERROR: Training dataset is empty after processing patches.")
            return DataLoader(EmptyDataset()), DataLoader(EmptyDataset())

        train_loader = DataLoader(
            train_dataset, batch_size=config['training_params']['batch_size'], shuffle=True, num_workers=0)
        val_loader = DataLoader(
            val_dataset, batch_size=config['evaluation_params']['eval_batch_size'], shuffle=False, num_workers=0)
...
class Trainer:
    ...
    def train(self, pretrain_cnn=False):
        """
        Main training controller. Dispatches to the correct training workflow.
        """
        # Save the master config before starting
        config_path = os.path.join(self.checkpoint_dir, 'config.json')

        # 1. Check if a config with a split already exists for this run.
        #    This is the logic for RESUMING a run.
        if os.path.exists(config_path):
            self.log("Found existing config.json, loading persistent data split.")
            with open(config_path, 'r') as f:
                self.config = json.load(f)
        else:
            # 2. If no config exists, this is a NEW run. Generate a split.
            self.log("No existing config found. Generating and saving a new data split.")
            self.config = self.data_manager.generate_data_split(self.config)
            # 3. Save the config WITH the new split immediately. This config will now be
            #    the single source of truth for all subsequent phases and resumes.
    ...
    def _train_end_to_end(self):
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=end_to_end_config['training_params']['learning_rate'], weight_decay=end_to_end_config['training_params']['weight_decay'])
    ...
    def _train_two_phase(self):
        ...
        cnn_optimizer = torch.optim.AdamW(cnn_model.parameters(), lr=pretrain_config['training_params']['learning_rate'], weight_decay=pretrain_config['training_params']['weight_decay'])
        ...
        if os.path.exists(best_cnn_path):
            ...
            self.log("WARNING: No best CNN model from Phase 1 found. Training GRU with random CNN weights.")
        ...
        fine_tune_optimizer = torch.optim.AdamW(param_groups, lr=main_lr, weight_decay=gru_train_config['training_params']['weight_decay'])
        
        self.log(f"Optimizer configured for fine-tuning. CNN LR: {main_lr/500:.1e}, GRU LR: {main_lr:.1e}")
        
        return self._run_training_loop(full_model, fine_tune_optimizer, phase_name="Phase2_FineTune")

    def _run_training_loop(self, model, optimizer, phase_name):
        """Generic training loop with phase-specific checkpointing and resuming."""
        # --- Setup paths and configs ---
        tp = self.config['training_params']
        num_epochs_total = tp.get('num_epochs', 30)
        
        if phase_name == "Phase1_CNN": num_epochs = max(1, num_epochs_total // 10)
        elif phase_name == "Phase2_GRU": num_epochs = num_epochs_total
        else: num_epochs = num_epochs_total
        
        config_path = os.path.join(self.checkpoint_dir, 'config.json')
        
        # Check if a config with a split already exists for this run
        if os.path.exists(config_path):
            self.log("Found existing config.json, loading persistent data split.")
            with open(config_path, 'r') as f:
                self.config = json.load(f)
        else:
            # If no config exists, this is a new run. Generate a split.
            self.log("No existing config found. Generating and saving a new data split.")
            self.config = self.data_manager.generate_data_split(self.config)
            # Save the config WITH the new split immediately.
            with open(config_path, 'w') as f:
                json.dump(self.config, f, indent=4)

        # Now, create dataloaders from the guaranteed-to-exist split in self.config
        train_loader, val_loader = self.data_manager.get_dataloaders(self.config)
        num_patches = len(train_loader.dataset)
        steps_per_epoch = (num_patches + tp['batch_size'] - 1) // tp['batch_size']
        total_train_steps = steps_per_epoch * num_epochs
        steps_per_checkpoint = max(1, steps_per_epoch // 2)

        latest_model_path = os.path.join(self.checkpoint_dir, f"latest_model_{phase_name}.pth")
        best_model_path = os.path.join(self.checkpoint_dir, f"best_model_{phase_name}.pth")
        ...
        if os.path.exists(latest_model_path):
            ...
            from collections import OrderedDict
            # 1. Clean the model state_dict keys
            model_state_dict = checkpoint['model_state_dict']
            new_state_dict = OrderedDict()
            for k, v in model_state_dict.items():
                name = k.replace('_orig_mod.', '') # remove `_orig_mod.` prefix
                new_state_dict[name] = v
            
            # 2. Load the cleaned state_dict
            ...
            self.log(f"Resumed from step {global_step}. Best validation loss so far: {best_val_loss:.4f}")

        if hasattr(torch, "compile"): model = torch.compile(model)
        self.log(f"[{phase_name}] Model has {sum(p.numel() for p in model.parameters() if p.requires_grad):,} trainable parameters.")
        self.log(f"[{phase_name}] Will train for {num_epochs} epochs ({total_train_steps} steps).")
        ...
        pbar = tqdm(initial=global_step, total=total_train_steps, desc=f"{phase_name} Steps")
        train_loss_accumulator = []

        while global_step < total_train_steps:
            # (Inner loop is identical to previous version)
            try: cqt_batch, f0_batch = next(train_iterator)
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16, enabled=use_amp):
                ...
                loss = torch.nn.functional.binary_cross_entropy_with_logits(logits, f0_batch)
            ...
            if (global_step + 1) % steps_per_checkpoint == 0 or (global_step + 1) == total_train_steps:
                avg_train_loss = sum(train_loss_accumulator) / len(train_loss_accumulator)
                metrics['train_loss'].append(avg_train_loss)
                train_loss_accumulator = []
                avg_val_loss = self._run_validation(model, val_loader, use_amp=use_amp)
                metrics['val_loss'].append(avg_val_loss)
                metrics['steps'].append(global_step + 1)
                self.log(f"Step {global_step+1}/{total_train_steps} | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f}")

                # Save best model
                if avg_val_loss < best_val_loss:
                    best_val_loss = avg_val_loss
                    torch.save(model.state_dict(), best_model_path)
                    self.log(f"  > New best val_loss for '{phase_name}'. Best model saved.")
    ...
    def _run_inference(self, log_hcqt):
        ...
        with torch.no_grad():
            for i, (batch,) in enumerate(loader):
                ...
                preds = torch.sigmoid(self.model(batch.to(self.device))).squeeze(1).cpu().numpy()
...
class SalienceStudioApp(ctk.CTk):
    ...
    def _create_train_tab(self):
        ...
        self.train_widgets['architecture_name'] = ctk.CTkOptionMenu(
            settings_frame, 
            values=model_names,
            command=self._on_model_select # Add command to check for SalienceNetV4
        )
        ...
        self.train_button.grid(row=1, column=0, padx=10, pady=0, sticky="ew")
        ...
        self.pretrain_button.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
        ...
        # Call the visibility check on startup
        self._on_model_select(self.train_widgets['architecture_name'].get())

    def _on_model_select(self, model_name):
        """Shows or hides the pre-training button based on the selected model."""
        if model_name.startswith("SalienceNetV4"):
            self.pretrain_button.grid() # Show the button
        else:
            self.pretrain_button.grid_remove() # Hide the button
    ...
    def start_training_thread(self, pretrain_cnn=False):
        ...
        self.config['run_id'] = config_hash
        ...
        # Pass the pretrain_cnn flag to the trainer's main method
        threading.Thread(target=self.run_training_and_get_results, args=(trainer, pretrain_cnn), daemon=True).start()
    ...
    def plot_training_loss(self, metrics):
        ...
        # Extract data for plotting, using .get() for safety
        steps = metrics.get('steps', [])
        train_loss = metrics.get('train_loss', [])
        val_loss = metrics.get('val_loss', [])

        # --- MODIFICATION: Update plotting logic ---
        fig, ax = plt.subplots(figsize=(6, 4), facecolor="#2B2B2B")
        
        # Plot train loss if data is available
        if steps and train_loss:
            ax.plot(steps, train_loss, label='Train Loss', color="#1F6AA5", marker='.', linestyle='-')
        
        # Plot validation loss if data is available
        if steps and val_loss:
            ax.plot(steps, val_loss, label='Val Loss', color="#FFA500", marker='.', linestyle='-')
        
        # Set plot labels and style
        ax.set_title("Training History", color="white")
        ax.set_xlabel("Training Steps", color="white") # More accurate x-axis label
        ...
        # Render the plot in the Tkinter canvas
